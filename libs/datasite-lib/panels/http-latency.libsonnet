...

{
  ...
  .addTarget(
    grafana.prometheus.target(
      expr = if global then 'irate('+metric+'_seconds_max{' + datasite.config.global_filter + '}[1m])' else 'irate('+metric+'_seconds_max{' + datasite.config.global_filter + ', uri=~"$'+metric+'"}[1m])',
      legendFormat = '{{ uri }}'
    )
  ),
}