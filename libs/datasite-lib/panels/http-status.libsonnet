...

{
  ...
  .addTarget(
    grafana.prometheus.target(
      expr = if global then 'sum by (status) ( rate('+metric+'_seconds_count{' + datasite.config.global_filter + '}[1m]) )' else 'sum by (status) ( rate('+metric+'_seconds_count{' + datasite.config.global_filter + ', uri=~"$'+metric+'"}[1m]) )',
      legendFormat = '{{ status }}'
    )
  ),
}