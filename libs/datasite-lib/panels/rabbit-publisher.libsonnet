...

{
  ...
  .addTarget(
    grafana.prometheus.target(
      expr = 'sum by (env) (rate(rabbitmq_published_total[1m]))',
      legendFormat = 'published total'
    )
  )
  .addTarget(
    grafana.prometheus.target(
      expr = 'sum by (env) (rate(rabbitmq_not_acknowledged_published_total[1m]))',
      legendFormat = 'not acknowledged published total'
    )
  )
  .addTarget(
    grafana.prometheus.target(
      expr = 'sum by (env) (rate(rabbitmq_acknowledged_published_total[1m]))',
      legendFormat = 'acknowledged published total'
    )
  )
  .addTarget(
    grafana.prometheus.target(
      expr = 'sum by (env) (rate(rabbitmq_failed_to_publish_total[1m]))',
      legendFormat = 'failed to publish total'
    )
  )
  .addTarget(
    grafana.prometheus.target(
      expr = 'sum by (env) (rate(rabbitmq_unrouted_published_total[1m]))',
